...
def Dummy(d):
    ...
    for i in d:
        for j in i:
            dummy += '(dummy a'+j[0]+'_'+j[1]+' d_'+j[0]+' a'+j[0]+'_d) '
    return dummy
...
def createGoal(Tdic):
    g = '(:goal (and '
    g += currentState(final_goal['cur_state'])
...
def createObjects(Tdic):
    o = '(:objects '
    o += dummyActivity(final_objects['dummy_activity'])
...
def startCustomFiles(tracesPath, constraintsPath):
    ...
    with open('FD_outputs/'+str(parent_folder)+'/Obtained averages.txt', "w", encoding="utf-8") as f:
        f.write("Total Time Average: "+str(averages)+'\nTotal Cost Average: '+str(averages_cost))

def createFolders(parent_folder, folder):
    try:
        os.makedirs('./pddl_problem_files/'+parent_folder)
    except:
        pass
    
    try:
        os.makedirs('./pddl_problem_files/'+parent_folder+'/'+folder)
    except:
        pass
    
    try:
        os.makedirs('./FD_outputs/'+parent_folder+'/'+folder)
    except:
        pass
        
    try:
        os.makedirs('./plot_data/'+parent_folder)
    except:
        pass 
...
def initConstraint(constraintsPath):
    ...
    final_objects = {'automaton_state': [], 'dummy_activity': [], 'activity': [] }
    final_init = {'automaton': [], 'dummy': [], 'cur_state': [], 'final_state': [] }
    final_goal = {'cur_state': [] }

    for j in range(1,len(allc)+1):

        objects = {'automaton_state': [], 'dummy_activity': [], 'activity': [] }
        init = {'automaton': [], 'dummy': [], 'cur_state': [], 'final_state': [] }
        goal = {'cur_state': [] }
        ...
        if len(final_states) == 1:
            ...
            objects['automaton_state'].append([str(j),'d'])
            objects['dummy_activity'].append([str(j)])
            goal['cur_state'].append([str(j),'d'])
            for i in final_states:
                objects['automaton_state'].append([str(j),i])
                init['final_state'].append([str(j),i])
                init['dummy'].append([str(j),i])
...
def initTrace(tracesPath, parent_folder, folder, constraintsPath):
    ...
    for x in range(0,num_traces):
        ...
        namefile = 'traceAlignmentProblem_'+str(x)
        ...
        with open('./pddl_problem_files/'+parent_folder+'/'+folder+'/'+namefile+'.pddl', "w", encoding="utf-8") as f:
            f.write(problemFile)
        print('----Trace Analized %i ----'%x)

    for i in range(0,num_traces):
        s = './fast-downward.py "/Users/Alessia/Documents/GitHub/ReasoningAgents21/pddl_domain_file/traceAlignmentDomain.pddl" "/Users/Alessia/Documents/GitHub/ReasoningAgents21/pddl_problem_files/'+str(parent_folder)+'/'+str(folder)+'/traceAlignmentProblem_'+str(i)+'.pddl" --search "astar(blind())" > /Users/Alessia/Documents/GitHub/ReasoningAgents21/FD_outputs/'+str(parent_folder)+'/'+str(folder)+'/FD_output'+str(i)+'.txt'
        p = subprocess.Popen(s, cwd="/Users/Alessia/downward/", shell=True)
...
def createPlotDataFile(parent_folder, folder, num_traces):
    ...
    filePlotData=open("/Users/Alessia/Documents/GitHub/ReasoningAgents21/plot_data/"+str(parent_folder)+"/plot_data_"+str(folder)+".txt", 'w+')
    ...
    for i in range(0,num_traces):
        ...
        s= 'FD_outputs/'+str(parent_folder)+'/'+str(folder)+'/FD_output'+str(i)+'.txt'
        f = open(s,'r')
        r = f.readlines()
